...
{
    public function index(Request $request, PaymentAlertService $alerts): View
    {
        $cliente = $this->cliente($request);
        $cliente->load(['ventas' => fn ($query) => $query->where('estado', '!=', 'anulada')->with('lote.manzano.urbanizacion', 'cuotas', 'cashMovements'), 'cashMovements']);

        return view('clientes.mi-cuenta', ['cliente' => $cliente, 'alertas' => $alerts->forCliente($cliente)]);
    }
